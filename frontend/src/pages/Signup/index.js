...
const SignUp = () => {
  ...
  // Detectar token do parceiro na URL
  ...
  const handleSignUp = async (values) => {
    ...
    // Se houver token do parceiro, incluir nos dados
    if (partnerToken) {
      Object.assign(values, { partnerToken: partnerToken });
    }
                  ...
                  >
                    {plans.map((plan, key) => (
                      <MenuItem key={key} value={plan.id}>
                        {plan.name} - {i18n.t("signup.plan.attendant")}:{" "}
                        {plan.users} - {i18n.t("signup.plan.whatsapp")}:{" "}
                        {plan.connections} - {i18n.t("signup.plan.queues")}:{" "}
                        {plan.queues} - R$ {plan.value}
                      </MenuItem>
                    ))}
                  </Field>
                </Grid>
